...
- (NSString * )stringDeleteZeroByfloatstring:(NSString * )str{
    if (str.length>0) {
        if ([str containsString:@"."]) {
            for (int i = 0; i<5; i++) {
                NSString * substring = [str substringWithRange:NSMakeRange(str.length-i-1,1)];
                if ([substring isEqualToString:@"0"]) {
                    continue;
                }else if([substring isEqualToString:@"."]){
                    return [str substringToIndex:str.length-i-1];
                }else{
                    return [str substringToIndex:str.length-i];
                }
            }
        }
        return str;
