...
//    [self.textQuantity resignFirstResponder];
    ...
    if (self.textQuantity.text.doubleValue>0) {
        ...
        if (![self isUsingCustomRoadAsset]) {
            NSIndexPath* indexPath=[self.roadAssetListView indexPathForSelectedRow];
            RoadAssetPrice *tempRoadAssetPrice = nil;
            if (indexPath!=nil) {
                tempRoadAssetPrice = [self.roadAssetWithLabel objectAtIndex:indexPath.row];
            }
            if ([self.textPrice.text isEqualToString:UNDETERMINED]) {
                newRoadAssetPrice = [RoadAssetPrice newDataObjectWithEntityName:@"RoadAssetPrice"];
                newRoadAssetPrice.name = tempRoadAssetPrice.name;
                newRoadAssetPrice.spec = tempRoadAssetPrice.spec;
                newRoadAssetPrice.unit_name = tempRoadAssetPrice.unit_name;
                newRoadAssetPrice.price = [NSNumber numberWithDouble:-1.0];
            }else{
                newRoadAssetPrice = tempRoadAssetPrice;
            }
        } else {
...
//弹出金额选择框
- (IBAction)selectTotalPrice:(id)sender {
    if (([self.pickerPopover isPopoverVisible])) {
        [self.pickerPopover dismissPopoverAnimated:YES];
    } else {
        
        PricePickerViewController *acPicker = [self.storyboard instantiateViewControllerWithIdentifier:@"PricePicker"];
        ...
        acPicker.data = [NSArray arrayWithObjects:UNDETERMINED, [NSString stringWithFormat: @"%.2f元", payPrice],nil ];
        ...
        [acPicker.tableView setFrame:CGRectMake(0, 0, 140, 100)];
        [self.pickerPopover presentPopoverFromRect:[(UITextField*)sender frame] inView:self.view permittedArrowDirections:UIPopoverArrowDirectionDown animated:YES];
