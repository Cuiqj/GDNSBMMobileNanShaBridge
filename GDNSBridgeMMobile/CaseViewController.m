...
#define CITIZENVIEW_HEIGHT 600.0
typedef enum {
...
- (void)viewDidLoad{
    ...
    self.citizenInfoBriefVC.view.frame=CGRectMake(0.0, 0.0, SCROLLVIEW_WIDTH, CITIZENVIEW_HEIGHT);
    ...
    self.deformInfoVC.view.frame=CGRectMake(0.0, 0.0, SCROLLVIEW_WIDTH,500.0);
    ...
    self.needsMove=NO;
     ...
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(toUpdateCaseInfo) name:@"toUpdateCaseInfo" object:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView.tag==kUITableViewTagDocTemplate) {
        ...
        if ([self.docTmplArray[indexPath.row] isEqualToString:@"施工整改通知书"]) {
            cell.textLabel.text = @"";
        }
...
- (IBAction)changeInfoPage:(UISegmentedControl *)sender {
    ...
    if (sender.selectedSegmentIndex!=5) {
        ...
        if (sender.selectedSegmentIndex==1) {
            self.infoView.contentSize=CGSizeMake(SCROLLVIEW_WIDTH, 600.0);
        } else {
            self.infoView.contentSize=CGSizeMake(SCROLLVIEW_WIDTH, SCROLLVIEW_HEIGHT);
        }
...
- (IBAction)btnNewCase:(id)sender;{
    ...
    NSInteger caseMark3InCoreData = [CaseInfo maxCaseMark3:yearString];
    caseMark3 = [[NSString alloc] initWithFormat:@"%d", caseMark3InCoreData + 1];
...
//            NSEntityDescription *entity=[NSEntityDescription entityForName:@"CaseInfo" inManagedObjectContext:context];
            if([[CaseInfo caseInfoByCaseMark2:self.textCasemark2.text withCaseMark3:self.textCasemark3.text ]count]> 0){
                UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"提示" message:@"案号重复" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
                [alter show];
                return;
            }
            if([self.textCaseDesc.text isEqualToString:@""]){
                [self AlertShowMessage];
                return;
            }
            self.caseInfo=[CaseInfo newDataObjectWithEntityName:@"CaseInfo"];
...
- (void)keyboardWillHide:(NSNotification *)aNotification{
    if (self.needsMove) {
        ...
        if (self.segInfoPage.selectedSegmentIndex==1) {
            self.infoView.contentSize=CGSizeMake(SCROLLVIEW_WIDTH, CITIZENVIEW_HEIGHT);
        } else {
            self.infoView.contentSize=CGSizeMake(SCROLLVIEW_WIDTH, SCROLLVIEW_HEIGHT);
        }        
...
- (void)keyboardWillShow:(NSNotification *)aNotification{
    if (self.needsMove) {
        ...
        CGSize tempSize;
        if (self.segInfoPage.selectedSegmentIndex==1) {
            tempSize=CGSizeMake(SCROLLVIEW_WIDTH, CITIZENVIEW_HEIGHT);
        } else {
...
- (void)AlertShowMessage{
     __weak typeof(self)weakself = self;
    UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"无法保存案件数据" message:@"请检查案件信息是否填写完整"  preferredStyle:UIAlertControllerStyleAlert];
...
//之前文书需要点击进去才会保存并上传到服务器，现在把文书的保存工作抽取出来放在这里，当保存案件信息的时候就会自动保存文书
- (void)saveCaseInfoForCase:(NSString *)caseID{
    NSArray *tempArray = [CaseInfo caseInfoByCaseMark2:self.textCasemark2.text withCaseMark3:self.textCasemark3.text];
    if([tempArray count] > 0){
        for(CaseInfo* caseinfo in tempArray){
            if(![caseinfo.myid isEqual:caseID]){
                UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"提示" message:@"案号重复" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
                [alter show];
                return;
            }
...
//    CaseProveInfo * caseProveInfo = [CaseProveInfo proveInfoForCase:caseID citizenName:self.textAutoNumber.text.uppercaseString];
    if (!caseProveInfo) {
        caseProveInfo=[CaseProveInfo newDataObjectWithEntityName:@"CaseProveInfo"];
        caseProveInfo.caseinfo_id=caseID;
        caseProveInfo.citizen_name = self.textAutoNumber.text.uppercaseString;
        if(caseProveInfo.start_date_time == nil){
            caseProveInfo.start_date_time=[[AppDelegate getFullDateFormatter] dateFromString:self.textHappenDate.text];
        }
        if (caseProveInfo.end_date_time == nil) {
            caseProveInfo.end_date_time=[[AppDelegate getFullDateFormatter] dateFromString:self.textHappenDate.text];
        }

        if([self.textSide.text rangeOfString:@"收费站" options:NSBackwardsSearch].location == 2 || [self.textPlace.text rangeOfString:@"匝道" options:NSBackwardsSearch].location == 1){
            caseProveInfo.remark = [NSString stringWithFormat:@"%@%@%@",self.textRoadSegment.text,self.textSide.text, self.textPlace.text];
        }else{
            caseProveInfo.remark = [NSString stringWithFormat:@"%@%@K%@+%@",self.textRoadSegment.text,self.textSide.text,self.textStationStartKM.text,[[AppConstants numberFormatter] stringFromNumber:[NSNumber numberWithInteger:[self.textStationStartM.text intValue]]]];
        }
    ...
    //即使多个当事人，但是案由也一定是相同的
    NSArray *array = [CaseProveInfo proveInfoByCaseId:caseID];
    for(CaseProveInfo *proveInfo in array){
        proveInfo.case_desc_id=caseDescID;
        proveInfo.case_short_desc = self.textCaseDesc.text;
    ...
    if (citizen1) {
        ...
        caseProveInfo.party_org_duty = [NSString stringWithFormat:@"%@%@",[citizen1.org_name length]==0?@"":citizen1.org_name, [citizen1.org_principal_duty length]==0?@"":citizen1.org_principal_duty];
    }
...
- (void)loadCaseProveInfoForCase:(NSString *)caseID{
    CaseProveInfo *info = [CaseProveInfo proveInfoForCase:caseID citizenName:self.textAutoNumber.text];
    if (info) {
...
- (void)saveCitizenInfoForCase:(NSString *)caseID andAutoNumber:(NSString *)aAutoNumber{
    aAutoNumber=[aAutoNumber stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].uppercaseString;
    self.textAutoNumber.text = aAutoNumber;
    self.citizenInfoBriefVC.textAutoNumber.text = self.textAutoNumber.text;
    if (![aAutoNumber isEmpty] && aAutoNumber != nil) {
        ...
        self.citizenInfo.automobile_pattern = self.textAutoPattern.text.uppercaseString;
        NSString *autoNumberNew = self.textAutoNumber.text.uppercaseString;
        //如果更新了车牌，则勘验检查笔录也需要更新车牌
        CaseProveInfo *caseProveInfo = [CaseProveInfo proveInfoForCase:caseID citizenName:self.citizenInfo.automobile_number];
        if(caseProveInfo){
            caseProveInfo.citizen_name = autoNumberNew;
        }
...
-(BOOL)checkCaseNum{
    if([CaseInfo caseInfoByCaseMark2:self.textCasemark2.text withCaseMark3:self.textCasemark3.text] > 0){
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"提示" message:@"案号重复" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alter show];
        return YES;
    }
...
// 更新停驶车辆的停驶地点
- (void)toUpdateCaseInfo{
    CaseInfo *caseInfo = [CaseInfo caseInfoForID:self.caseID];
    self.textSide.text = caseInfo.side;
}
@end
